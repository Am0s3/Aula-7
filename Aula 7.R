...
# Gráfico Transversal: Comparação entre Brasil e EUA em 2020
grafico_transversal <- ggplot(data = dadosELECTRICITY %>% filter(year == 2020), 
                              aes(x = country, y = EG.ELC.ACCS.ZS, fill = country)) +
...
# Exibir o gráfico
print(grafico_transversal)

# Gráfico Temporal: Comparação do Acesso à Eletricidade ao Longo do Tempo
grafico_temporal <- ggplot(data = dadosELECTRICITY, aes(x = year, y = EG.ELC.ACCS.ZS, color = country)) +
  geom_line(size = 1) +
  geom_point(size = 3) + 
  labs(title = "Evolução do Acesso à Eletricidade ao Longo do Tempo",
       x = "Ano",
       y = "Acesso à Eletricidade (%)",
       color = "País") +
  theme_minimal() +
